...
def slurm_wrapper_hydra(args, callable_fn, debug=False):
    ...
    if args.environment.slurm:
        ...
        args.environment.workers = max(10 * args.environment.ngpu, 10)
...
def slurm_wrapper(args, save_dir, func, func_kwargs, resubmit=True, func_args=()):
    ...
    if args.slurm:
        # asks SLURM to send USR1 signal 30 seconds before the time limit
        additional_parameters = {"signal": 'SIGUSR1@90', 'account': args.sl_account}
        executor = submitit.AutoExecutor(folder=save_dir)
        executor.update_parameters(
            tasks_per_node=args.sl_ntask_pnode,
            timeout_min=args.sl_time_hr*60,
...
def add_slurm_args(arg_parser):
    ...
    arg_parser.add_argument("--sl_time_hr",default=48, type=int, help='timeout in hours')  # in ours hrs
    arg_parser.add_argument("--sl_name", default='dev', type=str)  
    arg_parser.add_argument("--sl_gpu_mem_gb", default=None, type=str)  
    arg_parser.add_argument("--sl_dir", default=f'/move/u/{os.getenv("USER")}/slurm_cache', type=str)  
    ...
    arg_parser.add_argument("--sl_part",default='move,humanoid', type=str)
    arg_parser.add_argument("--sl_account",default='move', type=str)
    return arg_parser
...
def interactive_node():
    ...
    func = time.sleep
    slurm_wrapper(args, args.sl_dir, func, {}, True, (args.sl_time_hr*3600, ))
    return 
